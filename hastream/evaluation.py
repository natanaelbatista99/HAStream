...
class Evaluation(nn.Module):
    ...
    def evaluation_mensure(self):
        ...
        for tts in timestamps:
            ...
            df_partitions_mcs, df_partitions_hdbscan = self.flat_partitions(tts, 'mcs'), self.flat_partitions(tts, 'hdbscan')
            ...
            self.plot_heatmap(sub_dir, tts, df_partitions_mcs.shape[1] - 1, df_heatmap_ari, mensure)

            mensure = {'metric': 'nmi', 'label': 'Normalized Mutual Informatio'}
            self.plot_heatmap(sub_dir, tts, df_partitions_mcs.shape[1] - 1, df_heatmap_nmi, mensure)